#non ci sono queste logiche all'esame
import copy

class Model:
        def __init__(self):
            """
            Questo programma genera **tutti i quadrati magici** di dimensione NxN usando la **ricorsione**.

            Un quadrato magico è una disposizione di numeri da 1 a N² in una griglia N×N in cui:
            - la somma di ogni riga,
            - la somma di ogni colonna,
            - e la somma delle due diagonali principali
            sono tutte uguali a un numero chiamato **numero magico**.

            Il programma esplora tutte le permutazioni possibili dei numeri e verifica passo-passo (parzialmente)
            se le somme di righe, colonne o diagonali sono già fuori regola, per evitare di esplorare strade inutili.

            Alla fine stampa tutte le soluzioni trovate, il numero totale di soluzioni e il numero di iterazioni eseguite.
            """
